...
class p {
  constructor() {
    console.log(`NODE_ENV = ${process.env.NODE_ENV}`), this.apiBaseUrl = h[d].API_BASE_URL || null, this.endpoints = {
...
class f {
  ...
  async sendRestMessage(e, s) {
    try {
      ...
      return console.log(e, s), n.ok ? (await n.json()).response || null : (console.error("Failed to send REST message to Gemini."), null);
    } catch (n) {
